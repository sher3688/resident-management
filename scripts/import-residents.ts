 ...
 * 用法：tsx scripts/import-residents.ts <json_file_path>
 */
...
const importResidents = async (filePath: string) => {
  try {
    // 讀取 JSON 檔案
    ...
    let successCount = 0;
    let errorCount = 0;
    const errors: { index: number; unitNumber: string; ownerName: string; error: string }[] = [];
    ...
    for (let i = 0; i < data.length; i++) {
      ...
      try {
        ...
        const existing = await db
          .select()
          .from(residents)
          .where(eq(residents.unitNumber, resident.unitNumber))
          .limit(1);
...
if (args.length === 0) {
  console.log('用法：tsx scripts/import-residents.ts <json_file_path>');
  console.log('範例：tsx scripts/import-residents.ts /home/ubuntu/upload/B棟_converted.json');
  process.exit(1);
}
